...
void WriteVTPFile(vtkPolyData *writePolyData,std::string attachName)
{
  ...
  outputFilename = "/Users/adamupdegrove/Documents/Research/Code/vtkBoolean/Tests/"+attachName+".vtp";
      ...
      if (obbTree1->TriangleIntersectsNode
        ...
        for (vtkIdType id1 = 0; id1 < numCells1; id1++)
          ...
          if (type1 == VTK_TRIANGLE)
            ...
            // intersection line.
            if (info->Verbose)
	      std::cout<<"INTERSECTION of cells "<<cellId0<<" from Mesh 0 and "<<cellId1<<" from Mesh 1"<<endl;
	    ...
	    double surfaceid[2];
            int intersects =
              ...
              (triPts0[0], triPts0[1], triPts0[2],
               ...
               coplanar, outpt0, outpt1, surfaceid,tolerance,info->Verbose);
	    ...
	    //and surface maps!
            if ( intersects)
              ...
              if (info->Verbose)
	        std::cout<<"They did it "<<" LIne "<<lineId<<endl;
	      ...
	      if (addline)
		{
		  ...
		  for (vtkIdType edgeId = 0; edgeId < 3; edgeId++)
		    ...
		    {
                      if (info->Verbose)
		        std::cout<<"Point 1 on Mesh 1"<<endl;
		    }
		    ...
		    {
                      if (info->Verbose)
		        std::cout<<"Point 2 on Mesh 1"<<endl;
		    }
		    ...
		    {
                      if (info->Verbose)
		        std::cout<<"Point 1 on Mesh 2"<<endl;
		    }
		    ...
		    {
                      if (info->Verbose)
		      std::cout<<"Point 2 on Mesh 2"<<endl;
		    }
		    }
                  if (info->Verbose)
		    std::cout<<"MP values: "<<m0p0<<" "<<m1p0<<" "<<m0p1<<" "<<m1p1<<endl;
		  if (m0p0 > 0 && m1p0 > 0)
		    intersectionSurfaceId->InsertValue(ptId0,3);
		  if (m0p1 > 0 && m1p1 > 0)
		...
		cellIterLower = info->IntersectionPtsMap[0]->lower_bound(ptId0);
		cellIterUpper = info->IntersectionPtsMap[0]->upper_bound(ptId0);
                if (info->Verbose)
		  std::cout<<"CURRENT MAPPER"<<endl;
                while (cellIterLower != cellIterUpper)
               {
                 vtkIdType checkCellId = cellIterLower->second;
                 if (info->Verbose)
		   std::cout<<"CELL IDS AREEEEEE: "<<checkCellId<<endl;
		 ++cellIterLower;
	       }
              if (info->Verbose) 
	      {
		std::cout<<"Surface Id for pt "<<ptId0<<" is "<<intersectionSurfaceId->GetValue(ptId0)<<endl;
		std::cout<<"Surface Id for pt "<<ptId1<<" is "<<intersectionSurfaceId->GetValue(ptId1)<<endl;
		std::cout<<"Cells: "<<cellId0<<" and "<<cellId1<<endl;
	      }
              }
            }
          }
	    ...
	    int numCurrCells)
{
  ...
  if (this->Verbose)
    std::cout<<"InTeRsEcTiNg LiNeS"<<endl;
  while ( iterLower != iterUpper )
  {
    vtkIdType lineId = iterLower->second;
    if (this->Verbose)
      std::cout<<"LineID! "<<lineId<<endl;
	...
	interceptlines->InsertCellPoint( location->second );
        if (this->Verbose)
          std::cout<<"Second Adding Line "<<lines->GetNumberOfCells()<<
            " with point "<<ptIdMap[ linePtIds[i] ]<<endl;
        ...
        for (vtkIdType k = 0; k < nLinePts; k++)
        {
          ...
          while (ptIterLower != ptIterUpper)
          {
            ...
            while (cellIterLower != cellIterUpper)
            {
              ...
              if (this->Verbose)
	        std::cout<<"Check cell Ids for point "<<checkCellId<<endl;
...
//    	std::cout<<"Setting Boundary Point Bad "<<cellPts[ptId]<<endl;
  ...
  this->transformSign = this->GetTransform(transform,points);
    ...
    this->splittingPD->DeepCopy(fullpd);
...
//          std::cout<<"Inter Point Count! "<<interPtCount<<endl;
        ...
        if (this->Verbose)
	{
	  if (cellId == 39199 && inputIndex == 0)
	  {
	    WriteVTPFile(newpd,"PreDelaunay"+int2String(cellId)+"_"+
	    int2String(k));
	    WriteVTPFile(boundary,"PreDelaunayBoundary"+
	  int2String(cellId)+"_"+int2String(k));
	  }
	}
	...
	if (this->Verbose)
	{
	  if (cellId == 39199 && inputIndex == 0)
	  {
	    WriteVTPFile(del2D->GetOutput(),"NewLoopDelaunay"+
	    int2String(cellId)+"_"+int2String(k));
	  }
	}
                    ...
                    vtkIdType triPtIds[3])
{
  ...
  if (fabs(dist) < pow(this->Tol,3.0) && t >= 0.0 && t <= 1.0)
  //if (fabs(dist) <= 2*this->Tol && t >= 0.0 && t <= 1.0)
    {
...
int vtkIntersectionPolyDataFilter2::TriangleTriangleIntersection(double p1[3], 
					...
					double pt1[3], double pt2[3], double surfaceid[2],double tolerance,
					int verbose)
{
  double n1[3], n2[3];
  ...
  for (int i = 0; i < 3; i++)
    ...
    int val1 = vtkPlane::IntersectWithLine( pts1[id1], pts1[id2], n2, p2, t, x );
    if (verbose)
      std::cout<<"Inter 1 t: "<<t<<endl;
    if (val1 == 1 ||
    ...
    int val2 = vtkPlane::IntersectWithLine( pts2[id1], pts2[id2], n1, p1, t, x );
    if (verbose)
      std::cout<<"Inter 2 t: "<<t<<endl;
    if (val2 == 1 ||
	(t > (0-tolerance) && t < (1+tolerance)))
      {
	if (t < 1+tolerance && t > 1-tolerance)
...
void vtkIntersectionPolyDataFilter2::CleanAndCheckSurface(vtkPolyData *pd,double tol)
{
                                               ...
                                               vtkInformationVector*  outputVector)
{
  ...
  if (this->CheckInput)
  {
    ...
    this->CleanAndCheckInput(input0);
    std::cout<<"Checking Input 1"<<endl;
    this->CleanAndCheckInput(input1);
  }
  ...
  //vtkIndent indenter;
  //obbTree0->PrintSelf(std::cout,indenter);
  //double leaf_vol;
  //int minCells;
  //int maxCells;
  //obbTree0->DebugPrintTree(50,&leaf_vol,&minCells,&maxCells);
  ...
  //vtkSmartPointer<vtkPolyData> obbTree1Rep = 
  //vtkSmartPointer<vtkPolyData>::New();
  ...
  vtkSmartPointer<vtkIdFilter> addIds = vtkSmartPointer<vtkIdFilter>::New();
  addIds->SetInputData(outputIntersection);
  addIds->SetIdsArrayName("Ids");
  addIds->Update();
  outputIntersection->DeepCopy(addIds->GetOutput());
  ...
  if (outputIntersection->GetNumberOfLines() != tmpLines->GetNumberOfLines())
  {
    //vtkIdTypeArray *outputArray = vtkIdTypeArray::SafeDownCast(
    //    outputIntersection->GetCellData()->GetArray("Ids"));
    //for (vtkIdType lineId=0;lineId<outputIntersection->GetNumberOfLines();lineId++)
    //{
    //  if (outputIntersection->GetCellType(lineId) != VTK_LINE)
    //  {
    //      std::cout<<"Iters"<<endl;
    //    int value = outputArray->GetValue(lineId);
    //      std::cout<<"Iters"<<endl;
    //    outputIntersection->DeleteCell(lineId);
    //    IntersectionMapIteratorType it = impl->IntersectionMap[0]->begin();
    //    while (it != impl->IntersectionMap[0]->end())
    //    {
    //      if (it->second == value)
    //      {
    //        IntersectionMapIteratorType copyIt = it;
    //        ++it;
    //        impl->IntersectionMap[0]->erase(copyIt);
    //      }
    ...
    //        ++it;
    //        impl->IntersectionMap[0]->erase(copyIt);
    //        impl->IntersectionMap[0]->
    //          insert(std::make_pair(copyIt->first,copyIt->second-1));
    //      }
    //      else
    //        it++;
    //    }
    //    it = impl->IntersectionMap[1]->begin();
    //    while (it != impl->IntersectionMap[1]->end())
    //    {
    //      if (it->second == value)
    //      {
    //        IntersectionMapIteratorType copyIt = it;
    //        ++it;
    //        impl->IntersectionMap[1]->erase(copyIt);
    //      }
    //      if (it->second > value)
    //      {
    //        IntersectionMapIteratorType copyIt = it;
		  ...
		  this->NumberOfIntersectionLines == 0)
  {
    ...
    outputPolyData0->DeepCopy(impl->Mesh[0]);
    outputPolyData1->DeepCopy(impl->Mesh[1]);
    delete impl;
    return 0;
  }
  ...
  if ( this->SplitFirstOutput )
    ...
    if (this->CheckMesh)
      CleanAndCheckSurface(outputPolyData0,this->Tolerance);
  ...
  if ( this->SplitSecondOutput )
    ...
    if (this->CheckMesh)
      CleanAndCheckSurface(outputPolyData1,this->Tolerance);
...
::GetSingleLoop(vtkPolyData *pd,simPolygon *loop,vtkIdType nextCell,bool *interPtBool, bool *lineBool)
{
  ...
  while(nextPt != startPt)
  {
    ...
    {
      if (this->Verbose)
        std::cout<<"Found Point with more than two cells, need to find right one"<<endl;
      //This is the first intersection. Find line of minimum angle and 
      //set the orientation of the loop (i.e. CW or CCW)
      if (intertype == 0)
      {
        if (this->Verbose)
	  std::cout<<"Setting Loop Orientation"<<endl;
	...
	intertype = 1;
      ...
      {
        if (this->Verbose)
	  std::cout<<"Getting Loop Orientation"<<endl;
...
{           
  ...
  //area = orient2d(pt1,pt2,pt3);
  int orientation = 1;

  if (this->Verbose)
    std::cout<<"Area "<<area<< " for points "<<ptId1<<" "<<ptId2<<" "<<ptId3<<endl;
  if (fabs(area) < 1e-10)
  {
    ...
    std::cout<<"VERY SMALL AREA! Double Check the area"<<endl;
    ...
    {
      std::cout<<"NOT SAME TRANSFORM SIGN!!!"<<endl;
    ...
    vtkSmartPointer<vtkTransformPolyDataFilter> rotator = 
	    vtkSmartPointer<vtkTransformPolyDataFilter>::New();
    testPD->SetPoints(testPoints);
    testPD->SetLines(testCells);
    testPD->BuildLinks();
    rotator->SetInputData(testPD);
    rotator->SetTransform(newTransform);   
    rotator->Update();

    rotator->GetOutput()->GetPoint(0,pt1);
    rotator->GetOutput()->GetPoint(1,pt2);
    rotator->GetOutput()->GetPoint(2,pt3);
...
//Probably won't have this method because ear clipping is already implemented
//in vtkTriangleFilter
void vtkIntersectionPolyDataFilter2::Impl
...
//Clip the polygon if ear is found. This will probably also not be in the 
//full implementation because ear clipping is already implemented in 
//vtkTriangleFilter
int vtkIntersectionPolyDataFilter2::Impl
    ...
    vtkTransform *transform, vtkPoints *points)
{
  ...
  if (this->Verbose)
    std::cout<<"Dot of z axis! "<<dotZAxis<<endl;
  ...
  if (this->Verbose)
    std::cout<<"And angle is: "<<rotationAngle<<endl;
