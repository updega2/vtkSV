...
void WriteVTP(vtkPolyData *writePolyData,std::string attachName)
{
  ...
  outputFilename = 
   "/Users/adamupdegrove/Documents/Research/Code/vtkBoolean/Tests/"+attachName+".vtp";
...
void vtkBooleanOperationPolyDataFilter2::Impl::GetBooleanRegions(
    int inputIndex,std::vector<simLoop> *loops,int getOpenLoops)
{
  if (getOpenLoops ==1)
    std::cout<<"Getting Open Loop"<<endl;
  vtkIdType nextCell;
  vtkIdType p1,p2;
  ...
  for (loopit = loops->begin();loopit != loops->end(); ++loopit)
  {
    ...
    if (getOpenLoops == 0 && loopit->loopType == 2)
    {
      std::cout<<"Skipping Open Loop!"<<endl;
      continue;
    }
    //std::cout<<"GETTING LOOP WITH START POINT: "<<(loopit)->startPt<<endl;
    ...
    vtkInformationVector*  outputVector)
{
 ...
 ///////////////////////////////TMP/////////////////////////////////////////
  vtkSmartPointer<vtkIntArray> region1 = 
	  vtkSmartPointer<vtkIntArray>::New();
  vtkSmartPointer<vtkIntArray> region2 = 
	  vtkSmartPointer<vtkIntArray>::New();
  region1->SetName("FaceID");
  region2->SetName("FaceID");
  int mesh0Cells = impl->Mesh[0]->GetNumberOfCells();
  int mesh1Cells = impl->Mesh[1]->GetNumberOfCells();
  for (int i=0;i<mesh0Cells;i++)
	  region1->InsertNextValue(0);
  for (int i=0;i<mesh1Cells;i++)
	  region2->InsertNextValue(1);
  impl->Mesh[0]->GetCellData()->AddArray(region1);
  impl->Mesh[1]->GetCellData()->AddArray(region2);
 ///////////////////////////////TMP/////////////////////////////////////////
  ...
  for (loopit = loops.begin();loopit != loops.end(); ++loopit)
  {
    if ((loopit)->loopType != 0)
      std::cout<<"NOT REGULAR CASE"<<endl;
  }

  WriteVTP(impl->Mesh[0],"Surface0BooleanBefore");
  WriteVTP(impl->Mesh[1],"Surface1BooleanBefore");

  impl->GetBooleanRegions(0,&loops,0);
  if (impl->intersectionCase == 2)
    impl->GetBooleanRegions(0,&loops,1);
  std::cout<<"DONE WITH 1"<<endl;
  impl->GetBooleanRegions(1,&loops,0);
  if (impl->intersectionCase == 2)
    impl->GetBooleanRegions(1,&loops,1);
    ...
    std::vector<simLoop> *loops)
{
  ...
  for (interPt=0;interPt<numInterPts;interPt++)
  {
    if (usedPt[interPt] == false)
    {
      ...
      if (cellIds->GetNumberOfIds() > 2 || cellIds->GetNumberOfIds() < 2)
      {
      //  std::cout<<"Number Of Cells is less than or greater than 2 for first point "
      //    <<interPt<<endl;
	continue;
      }
	...
	if (this->intersectionCase == 2)
	{
      ...
      std::cout<<"NEW LOOP WITH START POINT "<<newloop.startPt<<" AND END POINT "<<newloop.endPt<<" ADDED"<<endl;
    }
  }
    ...
    vtkIdType interPt, vtkIdType nextCell, simLoop *loop, bool *usedPt)
{
  ...
  std::cout<<"NOOOOO LOOOOOOP FOUUUUNND"<<endl;
  return -1;
    ...
    vtkPolyData *output,int Operation)
{
  ...
  if (this->intersectionCase == 2)
  {
    ...
    for (int i=0;i<4;i++)
    {
      ...
      if (numCells[i] == 0)
      {
        std::cout<<"Found surface with only one son!"<<endl;
	std::cout<<"It is surface "<<i<<endl;
    ...
    if (nocellregion.size() > 0)
    {
      ...
      {
	std::cout<<"Swapping regions of surface 1"<<endl;
	...
	surfaces[0]->DeepCopy(tmp);
      ...
      {
	std::cout<<"Swapping regions of surface 2"<<endl;
