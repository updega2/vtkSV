/*=========================================================================
  ...
  Module:    vtkIntersectionPolyDataFilterOld.cxx
...
std::string getPa(std::string fullName)
{
  std::string pathName;
...
std::string getRawN(std::string fullName)
{
  std::string rawName;
  unsigned split = fullName.find_last_of("/\\");
...
void WriteVTPFi(std::string inputFilename,vtkPolyData *writePolyData,std::string attachName)
{
  std::string rawName, pathName, outputFilename;

  vtkSmartPointer<vtkXMLPolyDataWriter> writer  = vtkSmartPointer<vtkXMLPolyDataWriter>::New();

  pathName = getPa(inputFilename);
  rawName = getRawN(inputFilename);
...
//----------------------------------------------------------------------------
class vtkIntersectionPolyDataFilterOld::Impl
{
public:
  Impl();
  virtual ~Impl();
...
//----------------------------------------------------------------------------
vtkIntersectionPolyDataFilterOld::Impl::Impl() :
...
//----------------------------------------------------------------------------
int vtkIntersectionPolyDataFilterOld::Impl
::FindTriangleIntersections(vtkOBBNode *node0, vtkOBBNode *node1,
                            vtkMatrix4x4 *transform, void *arg)
{
  vtkIntersectionPolyDataFilterOld::Impl *info =
    reinterpret_cast<vtkIntersectionPolyDataFilterOld::Impl*>(arg);
      ...
      if (obbTree1->TriangleIntersectsNode
        ...
        for (vtkIdType id1 = 0; id1 < numCells1; id1++)
        {
          ...
          if (type1 == VTK_TRIANGLE)
          {
            ...
            double surfaceid[2];
            int intersects =
              vtkIntersectionPolyDataFilterOld::TriangleTriangleIntersection
...
::SplitMesh(int inputIndex,vtkPolyData *output,vtkPolyData *intersectionLines)
{
  ...
  if ( input->GetPolys()->GetNumberOfCells() > 0 )
  {
    ...
    for (cells->InitTraversal(); cells->GetNextCell(nptsX, pts); cellIdX++)
    {
      if ( nptsX != 3 )
      {
        vtkGenericWarningMacro( << "vtkIntersectionPolyDataFilterOld only works"
...
vtkCellArray* vtkIntersectionPolyDataFilterOld::Impl
            ...
            int numCurrCells)
{
  ...
  // vtkDelaunay2D_60 back to the original IDs in interLines. NOTE: The
  ...
  this->transformSign = this->GetTransform(transform,points);
	...
	if (cellId == 78918 && inputIndex == 1)
	{
          WriteVTPFi("/Users/adamupdegrove/Documents/Research/Code/vtkTests/Tests/PreDelaunay_"+intTString(cellId)+"_"+intTString(k)+".vtp",newpd,"Split");
          WriteVTPFi("/Users/adamupdegrove/Documents/Research/Code/vtkTests/Tests/PreDelaunayBoundary_"+intTString(cellId)+"_"+intTString(k)+".vtp",boundary,"Split");
          WriteVTPFi("/Users/adamupdegrove/Documents/Research/Code/vtkTests/Tests/Full_"+intTString(cellId)+"_"+intTString(k)+".vtp",fullpd,"Split");
	}
        ...
        if (cellId == 78918 && inputIndex == 1)
        {
          WriteVTPFi("/Users/adamupdegrove/Documents/Research/Code/vtkTests/Tests/PostDelaunay_"+intTString(cellId)+"_"+intTString(k)+".vtp",del2D->GetOutput(),"Post");
        }
       ...
       // // Renumber the point IDs.
        ...
        for (polys->InitTraversal(); polys->GetNextCell(npts, ptIds); )
        {
          ...
          if (interPtCount >= 2) //If there are more than two, inter line
          {
            ...
            //This is used in vtkBooleanOperationPolyDataFilterOld
    ...
    //  //Triangulate with delaunay2D
    //    if (cellId == 14 && inputIndex == 0)
    //    {
    //      WriteVTPFi("/Users/adamupdegrove/Documents/Research/Code/vtkTests/Tests/PreDelaunay_"+intTString(cellId)+".vtp",fullpd,"Pre");
    //    }
    ...
    //    if (cellId == 14 && inputIndex == 0)
    //    {
    //      WriteVTPFi("/Users/adamupdegrove/Documents/Research/Code/vtkTests/Tests/PostDelaunay_"+intTString(cellId)+".vtp",del2D->GetOutput(),"Post");
    //    }

    //  vtkCellArray *polys = del2D->GetOutput()->GetPolys();
    //  if (polys->GetNumberOfCells() != fullpd->GetNumberOfPoints() - 2)
    //  {
    //    int numoffsets = 1;
    //    while ((polys->GetNumberOfCells() != fullpd->GetNumberOfPoints()-2)
    //        && numoffsets < 20)
    //    {
    //      vtkSmartPointer< vtkDelaunay2D > del2Doffset =
    //        vtkSmartPointer< vtkDelaunay2D >::New();
    //      del2Doffset->SetInputData(fullpd);
    //      del2Doffset->SetSourceData(fullpd);
    //      del2Doffset->SetTolerance(0.0);
    //      del2Doffset->SetAlpha(0.0);
    //      del2Doffset->SetOffset(numoffsets);
    //      del2Doffset->SetProjectionPlaneMode(VTK_SET_TRANSFORM_PLANE);
...
void vtkIntersectionPolyDataFilterOld::Impl::AddToNewCellMap(
...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkIntersectionPolyDataFilterOld);

//----------------------------------------------------------------------------
vtkIntersectionPolyDataFilterOld::vtkIntersectionPolyDataFilterOld()
...
//----------------------------------------------------------------------------
void vtkIntersectionPolyDataFilterOld::PrintSelf(ostream &os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
int vtkIntersectionPolyDataFilterOld::TriangleTriangleIntersection(
...
void vtkIntersectionPolyDataFilterOld::CleanAndCheckSurface(vtkPolyData *pd,
    double stats[2],double tolerance)
{
  ...
  //Add normals at the end for proper display
...
void vtkIntersectionPolyDataFilterOld::CleanAndCheckInput(vtkPolyData *pd,
    double tolerance)
{
  vtkSmartPointer<vtkCleanPolyData> cleaner =
    vtkSmartPointer<vtkCleanPolyData>::New();
...
//----------------------------------------------------------------------------
int vtkIntersectionPolyDataFilterOld::RequestData(
                                        ...
                                        vtkInformationVector*  outputVector)
{
  ...
  // intersections.
  vtkIntersectionPolyDataFilterOld::Impl *impl =
    new vtkIntersectionPolyDataFilterOld::Impl();
  impl->Mesh[0]  = mesh0;
  impl->Mesh[1]  = mesh1;
  ...
  pointMerger->SetTolerance(this->Tolerance);
  ...
  obbTree0->IntersectWithOBBTree
    (obbTree1, 0, vtkIntersectionPolyDataFilterOld::
...
//----------------------------------------------------------------------------
int vtkIntersectionPolyDataFilterOld::FillInputPortInformation(int port,
                                                        vtkInformation *info)
{
...
void vtkIntersectionPolyDataFilterOld::Impl
...
int vtkIntersectionPolyDataFilterOld::Impl::GetLoopOrientation(
...
int vtkIntersectionPolyDataFilterOld::Impl::CheckLine(
...
int vtkIntersectionPolyDataFilterOld::Impl::GetTransform(
