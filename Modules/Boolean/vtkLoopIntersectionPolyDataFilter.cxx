/*=========================================================================
  ...
  Module:    vtkLoopIntersectionPolyDataFilter.cxx
...
#if DELAUNAY_TYPE == OLD
#include "vtkDelaunay2D_60.h"
#elif DELAUNAY_TYPE == TRIANGLE
...
//----------------------------------------------------------------------------
class vtkLoopIntersectionPolyDataFilter::Impl
{
public:
  Impl();
  virtual ~Impl();
...
public:
  ...
  vtkLoopIntersectionPolyDataFilter *ParentFilter;
...
//----------------------------------------------------------------------------
vtkLoopIntersectionPolyDataFilter::Impl::Impl() :
...
//----------------------------------------------------------------------------
int vtkLoopIntersectionPolyDataFilter::Impl
::FindTriangleIntersections(vtkOBBNode *node0, vtkOBBNode *node1,
                            vtkMatrix4x4 *transform, void *arg)
{
  vtkLoopIntersectionPolyDataFilter::Impl *info =
    reinterpret_cast<vtkLoopIntersectionPolyDataFilter::Impl*>(arg);
      ...
      if (obbTree1->TriangleIntersectsNode
        ...
        for (vtkIdType id1 = 0; id1 < numCells1; id1++)
          ...
          if (type1 == VTK_TRIANGLE)
            ...
            double surfaceid[2];
            int intersects =
              vtkLoopIntersectionPolyDataFilter::TriangleTriangleIntersection
...
::SplitMesh(int inputIndex, vtkPolyData *output, vtkPolyData *intersectionLines)
{
  ...
  if (input->GetPolys()->GetNumberOfCells() > 0)
    ...
    for (cells->InitTraversal(); cells->GetNextCell(nptsX, pts); cellIdX++)
      ...
      if (nptsX != 3)
        {
        vtkGenericWarningMacro(<< "vtkLoopIntersectionPolyDataFilter only works"
...
vtkCellArray* vtkLoopIntersectionPolyDataFilter::Impl
...
#elif DELAUNAY_TYPE == TRIANGLE
      vtkSmartPointer< vtkTriangleDelaunay2D > del2D =
        vtkSmartPointer< vtkTriangleDelaunay2D >::New();
#else
      ...
      if (polys->GetNumberOfCells() != newpd->GetNumberOfPoints() - 2)
        ...
        while ((polys->GetNumberOfCells() != newpd->GetNumberOfPoints()-2)
          ...
          vtkSmartPointer< vtkDelaunay2D_60 > del2Doffset =
            vtkSmartPointer< vtkDelaunay2D_60 >::New();
#elif DELAUNAY_TYPE == TRIANGLE
          vtkSmartPointer< vtkTriangleDelaunay2D > del2Doffset =
            vtkSmartPointer< vtkTriangleDelaunay2D >::New();
#else
...
void vtkLoopIntersectionPolyDataFilter::Impl::AddToNewCellMap(
...
void vtkLoopIntersectionPolyDataFilter::Impl
...
int vtkLoopIntersectionPolyDataFilter::Impl::GetLoopOrientation(
...
int vtkLoopIntersectionPolyDataFilter::Impl::CheckLine(
...
int vtkLoopIntersectionPolyDataFilter::Impl::GetTransform(
...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkLoopIntersectionPolyDataFilter);

//----------------------------------------------------------------------------
vtkLoopIntersectionPolyDataFilter::vtkLoopIntersectionPolyDataFilter()
...
//----------------------------------------------------------------------------
void vtkLoopIntersectionPolyDataFilter::PrintSelf(ostream &os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
int vtkLoopIntersectionPolyDataFilter::TriangleTriangleIntersection(
...
void vtkLoopIntersectionPolyDataFilter::CleanAndCheckSurface(vtkPolyData *pd,
...
void vtkLoopIntersectionPolyDataFilter::CleanAndCheckInput(vtkPolyData *pd,
    double tolerance)
{
  vtkSmartPointer<vtkCleanPolyData> cleaner =
    vtkSmartPointer<vtkCleanPolyData>::New();
...
//----------------------------------------------------------------------------
int vtkLoopIntersectionPolyDataFilter::RequestData(
                                        ...
                                        vtkInformationVector*  outputVector)
{
  ...
  // intersections.
  vtkLoopIntersectionPolyDataFilter::Impl *impl =
    new vtkLoopIntersectionPolyDataFilter::Impl();
  ...
  obbTree0->IntersectWithOBBTree
    (obbTree1, 0, vtkLoopIntersectionPolyDataFilter::
...
//----------------------------------------------------------------------------
int vtkLoopIntersectionPolyDataFilter::FillInputPortInformation(int port,
                                                        vtkInformation *info)
{
