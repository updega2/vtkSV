...
int main(int argc, char *argv[])
{
  if (argc != 4)
  {
      std::cout << "Need three surfaces: [Source S2 Surface] [Target Surface] [Target S2 Surface]!" <<endl;
      return EXIT_FAILURE;
  }
  ...
  vtkSmartPointer<vtkMapInterpolator> Mapper =
  ...
  std::string newOutName = getPath(inputFilename2)+"/"+getRawName(inputFilename2)+"/"+getRawName(inputFilename1);
  system(("mkdir -p "+newDirName).c_str());
  std::cout<<"Performing Operation..."<<endl;
  Mapper->SetInputData(0, pd1);
  Mapper->SetInputData(1, pd2);
  Mapper->SetInputData(2, pd3);
  Mapper->SetVerbose(1);
  Mapper->SetNumSourceSubdivisions(0);
  Mapper->Update();
  ...
  //vtkMakeParametricCube::WriteToGroupsFile(Mapper->GetOutput(0), newDirName+"/"+getRawName(inputFilename2)+"_GroupPoints");
  std::cout<<"Done"<<endl;
